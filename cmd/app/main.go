...
func main() {
	...
	body := flag.String("body", "Hello from Go over D-Bus!", "notification body")
	appName := flag.String("app", "furnace_driver", "application name")
	flag.Parse()
	...
	if call.Err != nil {
		log.Fatalf("failed to send notification: %v", call.Err)
	}

	if err := call.Store(&notificationID); err != nil {
		log.Fatalf("failed to decode notification id: %v", err)
	}
